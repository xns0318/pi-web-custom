...
test("native PTY starts after install and repeated creation with the same ID reuses its process", (t) => {
...
test("different IDs in the same cwd own independent PTYs and can be terminated separately", (t) => {
  const firstId = createTerminal(process.cwd(), 80, 24);
  t.after(() => killTerminal(firstId));
  const secondId = createTerminal(process.cwd(), 80, 24);
  t.after(() => killTerminal(secondId));
  const first = globalThis.__piWebTerminals.get(firstId);
  const second = globalThis.__piWebTerminals.get(secondId);
  assert.notEqual(firstId, secondId);
  assert.notEqual(first.pty.pid, second.pty.pid);
  assert.equal(getTerminalCwd(firstId), getTerminalCwd(secondId));
  assert.equal(createTerminal(process.cwd(), 100, 30, secondId), secondId);
  assert.strictEqual(globalThis.__piWebTerminals.get(secondId), second);
  killTerminal(firstId);
  assert.equal(hasTerminal(firstId), false);
  assert.equal(hasTerminal(secondId), true);
