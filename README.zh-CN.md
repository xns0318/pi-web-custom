...
> 基于 [agegr/pi-web](https://github.com/agegr/pi-web) 正式版 **v0.9.1** 的个人定制版本。
> 保留上游开发历史、版权声明和 MIT 许可证；本仓库独立维护，按需同步上游更新，
> 不代表上游官方发行版本，也不以上游合并这些定制为前提。
...
整合范围、测试结果和已知限制见 [v0.9.1 整合说明](./docs/upgrades/v0.9.1-local.md)。

## 运行这个定制版本

要求 Node.js 22.19.0 或更高版本。在新目录中构建，不要在已有 dev 服务运行的目录中执行构建：
...
可通过 `PI_CODING_AGENT_DIR` 沿用已有智能体数据；不要让多个服务同时操作同一会话的智能体任务。
...
- **Token 用量统计**：在“工具”右侧打开“用量”，查看今日、近 7/30 天、全部历史，以及逐日、按模型、按项目明细；fork 历史去重。服务端后台每 30 秒更新，也可手动立即刷新。详见[统计口径](./docs/token-usage.md)。
- **两种分支方式**：**新会话**会从较早的消息创建独立会话文件；**从此处编辑**会在当前会话内创建分支。
- **项目文件工具**：浏览、上传和预览文件、查看 Git Diff；支持 UTF-8 文本手动编辑保存、切换文件保留草稿和冲突检查，以及当前工作目录内新建、重命名、永久删除文件或文件夹。详见[文件编辑与管理](./docs/workspace-files.md)。
- **Git worktree**：从侧边栏切换 checkout，同时把同一仓库不同 worktree 的会话归在一起。
- **网页配置**：无需离开 Pi Web，即可管理 Provider 登录和 API Key、模型、模型测试、插件包及技能。
- **英文、简体中文和繁体中文界面**：Pi Web 首次打开时跟随浏览器语言，也可在设置中切换语言和主题。
